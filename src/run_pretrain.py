...
def read_data(args, tokenizer: BertTokenizer) -> dict:
    train_path = os.path.join(args.pretrain_data_path, 'train.json')
    dev_path = os.path.join(args.pretrain_data_path, 'dev.json')
    ...
    for i, row in tqdm(dev_df.iterrows(), desc='', total=len(dev_df)):
        content, label = row[0], row[1]
        content = content.split('\002')
        inputs_dict = tokenizer.encode_plus(content, add_special_tokens=True,
                                            return_token_type_ids=True, return_attention_mask=True)
